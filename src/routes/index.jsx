...
// 엑셀자료 업로드
import AdminExcelWork from 'pages/admin/excelwork/AdminExcelWork';
import AdminExcelWork2 from 'pages/admin/excelwork/AdminExcelWork2';
import AdminExcelWork3 from 'pages/admin/excelwork/AdminExcelWork3';
import AdminExcelWork4 from 'pages/admin/excelwork/AdminExcelWork4';
import AdminExcelWork5 from 'pages/admin/excelwork/AdminExcelWork5';
import AdminExcelWork6 from 'pages/admin/excelwork/AdminExcelWork6';
...
const RootRoutes = () => {
  ...
  //리액트에서 관리자메뉴자에 접근하는 토큰값 위변조 방지용으로 서버에서 비교하는 함수 추가
...
const SecondRoutes = () => {
  ...
  //useRef객체를 사용하여 페이지 마운트 된 후 ui.js를 로딩 하도록 변경 코드 추가(아래) 0910 마운트 된 후 불러올 경우 js 파일 못 불러오는 것으로 확인되어 주석처리
  // const isMounted = useRef(false); // 아래 로그인 이동 부분이 2번 실행되지 않도록 즉, 마운트 될 때만 실행되도록 변수 생성
  useEffect(() => {
    initPage();
  //   if (!isMounted.current) { // 컴포넌트 최초 마운트 시 페이지 진입 전(렌더링 전) 실행
  ...
  return (
    <>
      ...
      {/* 엑셀 업로드 기능 */}
      <Route path={URL.ADMIN_EXCELWORK} element={<AdminExcelWork mode={CODE.MODE_CREATE} />} />
      <Route path={URL.ADMIN_EXCELWORK2} element={<AdminExcelWork2 mode={CODE.MODE_CREATE} />} />
      <Route path={URL.ADMIN_EXCELWORK3} element={<AdminExcelWork3 mode={CODE.MODE_CREATE} />} />
      <Route path={URL.ADMIN_EXCELWORK4} element={<AdminExcelWork4 mode={CODE.MODE_CREATE} />} />
      <Route path={URL.ADMIN_EXCELWORK5} element={<AdminExcelWork5 mode={CODE.MODE_CREATE} />} />
      <Route path={URL.ADMIN_EXCELWORK6} element={<AdminExcelWork6 mode={CODE.MODE_CREATE} />} />
